...
Порядок и сопоставление ответов сохранены: обе стороны обрабатывают сообщения строго по очереди, как и раньше. При ошибке посреди окна хвост уже отправленных пакетов аккуратно поглощается, чтобы не быть принятым за ответы следующего файла. Медленный диск на приёме ограничивает буфер тем же окном — память не растёт.
