...
**Звук с удалённого компьютера не передавался никогда.**
Захват звука не работал вовсе: каждая попытка запуска завершалась ошибкой, и в журнале это повторялось при каждом подключении. Причина: захват выполняется в отдельном потоке, а COM требуется инициализировать в каждом потоке отдельно — здесь этого не делалось нигде. Обнаружено по журналам с тестовой машины, где подряд шло четырнадцать одинаковых неудач.
Исправление: COM инициализируется на всё время жизни потока захвата. Если инициализация не удастся, в журнал теперь пишется прямое объяснение — прежде причину приходилось выводить из кода ошибки.
